...
class APICalls:
    ...
    def bond_rate_call(self):
        ...
        """
        response = requests.get("https://www.quandl.com/api/v3/datasets/USTREASURY/YIELD.json?api_key=" +
                                self.quand_key)
    ...
    def options_expiration_call(self, ticker):
        ...
        if response.status_code == 200:
            ...
            print("The following ticker failed to get Options Expiration with the following error: ",
    ...
    def options_date_call(self, ticker, expiration):
        ...
        Calls and returns passed security's option expiration list from IEXCloud
        """
        response = requests.get("https://cloud.iexapis.com/stable/stock/" + ticker + "/options/" +
                                expiration + "?token=" + self.iex_key)

        if response.status_code == 200:
            return response.json()
        else:
            print("The following ticker for options failed with the following error: ", ticker, expiration,
    ...
    def historical_volatility_call(self, ticker):
        ...
        """
        response = requests.get("https://cloud.iexapis.com/stable/stock/" + ticker +
                                "/indicator/volatility/?range=1y&indicatorOnly=true&token=" + self.iex_key)
    ...
    def iv_rank_call(self, ticker, current, low, high):
        ...
        """
        response = requests.get('https://alarichartsock.com/api/' + str(current) + '/' + str(low) + '/' + str(high) + '/')
